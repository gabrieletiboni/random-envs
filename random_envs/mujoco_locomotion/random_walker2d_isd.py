...
class RandomWalker2dIsdEnv(MujocoEnv, utils.EzPickle):
    ...
    def reset_model(self):
        ...
        low = np.array([0.] + [-.4] + [-1.04] + ([-1.04] + [-.53] + [-.53])*2 + [0.] + [-.4] + [-1.] + [-1.]*3*2)*self.isd_randomness
        high = np.array([0.] + [0.5] + [1.04] + ([1.04] + [.53] + [.53])*2 + [2.] + [5.] + [1.] + [1.]*3*2)*self.isd_randomness

        self.set_state(
            self.init_qpos + self.np_random.uniform(low=low[:9], high=high[:9]),
            self.init_qvel + self.np_random.uniform(low=low[9:], high=high[9:])
        )        
...
gym.envs.register(
        id="RandomWalker2dIsd-v0",
        entry_point="%s:RandomWalker2dIsdEnv" % __name__,
